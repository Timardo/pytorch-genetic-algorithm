...
with torch.no_grad():
    ...
    start_times_vec = torch.tensor([int(line.split(";")[6]) for line in csvBusLinesFileLines if len(line) > 0], device="cpu")
    end_times_vec = torch.tensor([int(line.split(";")[7]) for line in csvBusLinesFileLines if len(line) > 0], device="cpu")
    dist_matrix = torch.tensor([[int(value) for value in csvDistMatrixFile.readline().split(";") if value != "\n"] for j in range(bus_line_count)], device="cpu")
    transition_matrix_negated = end_times_vec.unsqueeze(-1).expand(bus_line_count, bus_line_count).add(dist_matrix).gt(start_times_vec).to(device=device)
    ...
    population_matrix = torch.zeros(population_size, bus_line_count, starting_buses_count, device=device, dtype=torch.int16)
    ...
    population_indices_1 = torch.arange(population_size, device=device).unsqueeze(-1).unsqueeze(-1).expand(population_size, starting_buses_count, bus_line_count).int()
    # indices matrix used to create 4D matrix that defines current transitions between bus lines
    row_indices_1 = torch.arange(starting_buses_count, device=device).unsqueeze(-1).expand(starting_buses_count, bus_line_count).int()
    ...
    population_matrix_basic = [[0 for j in range(bus_line_count)] for i in range(population_size)]
    ...
    ############################################################
    # New code that does not use temporary structures but rather does everything in one line to save space and increase performance
    ############################################################
    fitness = population_matrix_transposed[population_indices_1, row_indices_1].bitwise_and_(population_matrix_transposed[population_indices_1, row_indices_1].transpose_(2, 3)).triu_(diagonal=1).bitwise_and_(transition_matrix_negated).cumsum_(2).cumsum_(3)[:, :, bus_line_count - 1, bus_line_count - 1].cumsum_(1)[:, starting_buses_count - 1].add_(population_matrix.cumsum_(1).gt_(0).cumsum_(2)[:, bus_line_count - 1, starting_buses_count - 1])
    ...
    for x in range(max_generations):
        ...
        population_mutation_indices = torch.rand(population_size, bus_line_count, device=device).mul_(buses_counts.cumsum_(1)[:, starting_buses_count - 1].unsqueeze(-1).expand(population_size, bus_line_count)).int()
        ...
        # mutated_bus_matrix = torch.rand(POPULATION_SIZE, bus_line_count, device=device).mul_(STARTING_BUSES_COUNT).mul_(population_mutation_mask).int()
        # flip the mask
